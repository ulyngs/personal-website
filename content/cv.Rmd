...
Download [my CV as PDF](/pdfs/2020_nov_academic_cv.pdf).
...
cv_entries %>% 
  ...
  mutate(what = if_else(
    ...
    str_c(what, ", ", where, "<br>", additional_info),
    str_c(what, ", ", where))
  ) %>% 
  mutate(year = year(date_begin)) %>% 
  select(year, what) %>% 
...
talks_table <- cv_entries %>% 
  ...
  mutate(what = ifelse(is.na(slides),
                       str_c("<em>", what, "</em>, ", where),
                       str_c("<em>", what, "</em>, ", where, "<br><a href=", slides, ">Slides</a>"))) %>%   
...
poster_table %>% kable(align = c("l", "l"))
cat('</div>')

# media tab
cat('<div class="tab-pane container fade" id="media">')
media_table %>% kable(align = c("l", "l"))
cat('</div>')
...
cv_entries %>% 
  ...
  mutate(what = ifelse(!is.na(additional_info),
                       str_c("<span class='work-title'>", what,
...
cv_entries %>% 
  ...
  mutate(what = str_c("<i class='fa fa-lg ", icon, "'></i> &nbsp;", what, " ", additional_info)) %>% 
  select(type, what) %>% 
  mutate(type = "") %>% 
  kable(escape=FALSE)
```
