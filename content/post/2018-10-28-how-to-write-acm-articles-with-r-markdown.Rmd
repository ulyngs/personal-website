...
categories:
  ...
  - reproducibility
  - ACM CHI
teaser: "/post/2018-10-28-how-to-write-acm-articles-with-r-markdown/preview.png"

---

*NOTE: After writing this post, I [created two R packages](https://ulriklyngs.com/blog/r-packages-for-chi-papers-with-r-markdown) which add R Markdown templates for [CHI Proceedings](https://github.com/ulyngs/chi-proc-rmd-template) and [CHI Extended Abstract](https://github.com/ulyngs/chi-ea-rmd-template) manuscript to RStudio. They implement the steps detailed in this post (and more) and provide examples within the sample content.*
...
To use the ACM article template, we'd normally download the latest version [from here](https://www.acm.org/publications/proceedings-template). For the purposes of this tutorial, however, clone or download [this GitHub repo](https://github.com/ulyngs/acm-articles-rmd) which I set up with just the files you need to follow along and understand how things work. Open the R Project file in the repo's root folder. Now let's have a look at the files in the **step1** folder:

<img class="img-responsive" alt="Step 1 files" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step1-files.png">
...
If you click 'Knit', the CHI proceedings template with sample content should compile to this pdf (if you get an error message, its probably for [this reason](https://github.com/jgm/pandoc/issues/970)):

<img class="img-responsive" alt="Step 1 compiled to pdf" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step1-compiled.png">
...
<img class="img-responsive" alt="Step 2 compiled first page" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step2-compiled-page1.png">

Ok, so title and abstract is plugged in correctly from the YAML header. Let's look at page 2:

<img class="img-responsive" alt="Step 2 compiled second page" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step2-compiled-page2.png">
...
If we add `keep_tex: true` in the YAML header, as an indented line under `bookdown::pdf_document:`, we can knit again and inspect the intermediary .tex file that the PDF is generated from. We might notice the following in the .tex file:

````markdown
(...) playable track for Guitar Hero Live.(Hume 1748)
````


and in the part containing the references:
...
<img class="img-responsive" alt="Step 3 compiled to pdf" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step3-compiled.png ">
...
```markdown
(...) playable track for Guitar Hero Live.\citep{Hume1748}
```
...
````markdown
`r ''````{r tribute-plot, echo=FALSE, out.width='0.98\\columnwidth', fig.cap="This is a column-width plot of how great Tribute gets over time"}
...
<img class="img-responsive" alt="Step 4 compiled to column width" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step4-colwidth.png">
...
````markdown
`r ''````{r two-col-tribute-plot, fig.env='figure*', echo=FALSE, out.width='0.98\\textwidth', fig.cap="This is a two-column plot of how great Tribute gets over time"}
...
<img class="img-responsive" alt="Step 4 compiled to full width" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step4-two-colwidth.png">
...
````markdown
`r ''````{r setup, include=FALSE, message=FALSE}
library(tidyverse)
```
````
...
<img class="img-responsive" alt="Step 5 column width" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step5-colwidth.png">

Awesome! No more errors from whatever obscure means through which you'd otherwise be getting your data into a LaTeX table.

## Two-column tables
Finally, the acm-article documentation again tells us that we can get our table to span two columns by putting it in a `\table*` rather than a `\table`environment. <del>It turns out that there's not at the moment any easy way to do this from within R Markdown (the `fig.env` chunk option doesn't work with tables when we use the kable function).</del>

*Update 7 Aug 2018:* Yihui Xie, creator of knitr, [told me](https://github.com/yihui/knitr/issues/1585#issuecomment-410922084) that this is done just as easily. We simply add `table.env='table*'` as argument to the kable function:

````markdown
`r ''````{r table-iris-wide, echo=FALSE}
iris %>%
  select(-Species) %>%  #drop the species column
  head(10) %>%          #show just the first 10 rows
...
Et voilá, our table is put in a `\table*` environment in the .tex file and now spans two columns:
<img class="img-responsive" alt="Step 5 full width" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step5-two-col-width.png">
...
```markdown
(...) over time is shown in Figure \ref{fig:col-width-tribute-plot}.
```
...
```markdown
(...) over time is shown in Figure \@ref(fig:tribute-plot).
```

Either way, the .tex output is the same, and so is the result:
<img class="img-responsive" alt="Step 6 referencing figures and tables" src="https://raw.githubusercontent.com/ulyngs/acm-articles-rmd/master/figures/step6.png">
