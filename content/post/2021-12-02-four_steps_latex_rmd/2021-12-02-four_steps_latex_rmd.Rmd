...
Then create a new R Markdown document, tell it to use the **.tex** file for PDF output, and try to knit it.
...
Ask yourself this: What and where do you want to plug in stuff from the YAML header (i.e., the stuff in between `---` at the top)? And where do you want to plug in your body content?
...
The variable names in your YAML header can be complete arbitrary - they don't have to be something that `pandoc` has seen before.
Let's pull in also the abstract and the author summary, from the R Markdown file, and use silly names:
...
The body content is pulled in with a special variable: if you write `$body$` somewhere in the template, pandoc will insert your body content there.

In our example with the PLOS template, try replacing everything from the introduction to end of the document with simply `$body$`. So
